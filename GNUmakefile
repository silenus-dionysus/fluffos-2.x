...
OBJ=$(addprefix $(OBJDIR)/,$(subst .c,.bc,$(SRC)))

$(OBJDIR)/%.bc: %.c 
	$(CXX) $(CFLAGS) $(OPTIMIZE) -c -emit-llvm $< -o $@

$(OBJDIR)/lex.bc: lex.c preprocess.c cc.h grammar.tab.c
	$(CXX) $(CFLAGS) $(OPTIMIZE)  -c -emit-llvm $< -o $@

$(OBJDIR)/grammar.tab.bc: grammar.tab.c opcodes.h
	$(CXX) $(CFLAGS) $(OPTIMIZE)  -c -emit-llvm $< -o $@
...
$(DRIVER_BIN): packages/packages.a $(OBJ) dtrace_compile
	...
	llvm-link $(OBJ) -o combined.bc
	$(PROOF) $(CXX) $(CFLAGS) $(OPTIMIZE) combined.bc `./dtrace_compile` -o $(DRIVER_BIN) packages/packages.a $(EXTRALIBS) `cat system_libs`

dtrace_compile: dtrace_compile.c local_options
	$(CXX) dtrace_compile.c -o dtrace_compile

addr_server: files $(OBJDIR)/addr_server.bc $(OBJDIR)/socket_ctrl.bc $(OBJDIR)/port.bc addr_server.h
	llvm-link $(OBJDIR)/socket_ctrl.bc $(OBJDIR)/addr_server.bc $(OBJDIR)/port.bc \
	-o combined.bc
	$(CXX) $(CFLAGS) $(OPTIMIZE) combined.bc -o addr_server `cat system_libs`

portbind: $(OBJDIR)/portbind.bc
	$(CXX) $(CFLAGS) $(OPTIMIZE) $(OBJDIR)/portbind.bc -o portbind `cat system_libs`
...
$(OBJDIR)/edit_source.bc: edit_source.c preprocess.c cc.h

edit_source: $(OBJDIR)/edit_source.bc $(OBJDIR)/hash.bc $(OBJDIR)/make_func.tab.bc
	llvm-link $(OBJDIR)/edit_source.bc $(OBJDIR)/hash.bc $(OBJDIR)/make_func.tab.bc -o combined.bc
	$(CXX) combined.bc -o edit_source

# don't optimize these two
$(OBJDIR)/edit_source.bc: edit_source.c
	$(CXX) $(CFLAGS) -c -emit-llvm $< -o $@
	
$(OBJDIR)/make_func.tab.bc: make_func.tab.c
	$(CXX) $(CFLAGS) -c -emit-llvm $< -o $@
