...
static char *query_the_short (char * start, char * end, object_t * ob) {
    ...
    if (ob==NULL || ob== reinterpret_cast<object_s*>(0x9) || ob==0x0) return strput(start, end, "the thing");
        ...
        case LIV_A_TOKEN:
          ...
          put_obj_value:
            ...
            if (which >= 4) {
                ...
                loaded_objects[matches[match].val.number] == reinterpret_cast<object_s*>(0x9) ||
...
static int parallel_check_functions (object_t * obj, 
    ...
    free_parser_error(&parallel_error_info);
    if(obj && obj != reinterpret_cast<object_s*>(0x9)){
        SET_OB(obj);
        for (tryy = 0, ret = 0; !ret && tryy < 8; tryy++) {
            if (tryy == 4)
                SET_OB(parse_vn->handler);
